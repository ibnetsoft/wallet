...
### 6단계: 사용자 PWA 모바일 웹앱 (Next.js Client Web) 개발
일반 고객들이 스마트폰 홈 화면에 바로가기를 추가하여 하단 탭 바 인터페이스(홈, 지갑, 게임, 조직, 설정)를 통해 가상자산을 조회하고 스왑하며 369 계보도를 시각적으로 확인할 수 있는 환경을 빌드합니다.
- [ ] **Next.js PWA 초기 세팅 및 App Shell 네비게이션 구축**
  - `manifest.json`, Service Worker 등록 및 iOS 전용 메타 태그 설정.
  - 하단 탭 바 UI 렌더링 (홈, 지갑, 게임, 조직, 설정).
- [ ] **조직(Network) 탭 이원화 및 369 롤업 시각화 개발**
  - 추천계보도(직속 1대 리스트)와 후원계보도(369 롤업 룰 반영 배치 트리) 탭 분리.
  - SVG 계층 다이어그램을 활용해 롤업/패스업 노드 시각화.
- [ ] **입금 QR 스캐너 & 클립보드 연동**
  - HTML5 Camera API 또는 QR 리더 모듈 연동 및 클립보드 복사 UX 제공.
- [ ] **스왑 & 송금 인터페이스 구현**
  - 0.1% 스왑 수수료가 적용된 URC ↔ USDT 스왑 및 $50 이상 출금 신청 화면 구성.
