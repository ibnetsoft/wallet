...
export async function GET() {
  ...
  try {
    ...
    for (const candidate of candidatesRes.rows) {
      ...
      try {
        ...
        try {
            ...
            await runClient.query(
              `UPDATE public.auto_bet_settings
               SET rounds = array_remove(rounds, $2::int),
                   enabled = CASE
                     WHEN cardinality(array_remove(rounds, $2::int)) = 0 THEN FALSE
                     ELSE enabled
                   END,
                   updated_at = NOW()
               WHERE user_id = $1`,
          ...
          const errorMessage = error instanceof Error ? error.message : "Auto bet failed";
          const errorCode = error instanceof ParticipationError ? error.code : "AUTO_BET_FAILED";

          await runClient.query(
            `UPDATE public.auto_bet_executions
             SET status = 'FAILED', error_message = $4
             WHERE user_id = $1 AND round_id = $2 AND round_date = $3::date`,
            [candidate.user_id, candidate.round_id, today, `${errorCode}: ${errorMessage}`]
          );
          await runClient.query("COMMIT");
    ...
    return NextResponse.json(
      { success: false, error: error instanceof Error ? error.message : "Auto bet failed" },
      { status: 500 }
    );
  } finally {
    client.release();
