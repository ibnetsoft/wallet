...
import { BarChart3, CalendarRange, RefreshCw, ShoppingBag } from "lucide-react";
...
interface DailyBettingResponse {
  ...
  summary?: {
    ...
    daysWithData: number;
    averageDailyCombinedAmount: number;
...
export default function BettingDailyPage() {
  ...
  const fetchDailyTotals = async (targetDays: number) => {
    ...
    try {
      const response = await fetch(adminApi(`/api/betting-daily?days=${targetDays}`), {
        cache: "no-store",
      });
    ...
    <div className="space-y-8 font-sans">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-[#8E8E93]">
            날짜별 상품구매총액과 게임 배팅총액을 함께 확인합니다. 기준 단가는 1티켓당 {unitBetAmount} USDT입니다.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 xl:grid-cols-4">
        <div className="rounded-2xl border border-[#26262B] bg-[#16161A] p-6 shadow-lg">
          <div className="mb-4 flex items-center justify-between">
            <span className="text-xs font-semibold uppercase tracking-wider text-[#8E8E93]">기간 총 상품구매액</span>
            <ShoppingBag size={18} className="text-[#F0B90B]" />
          </div>
          <div className="font-mono text-2xl font-extrabold text-white">
            {formatUsdt(summary?.totalProductPurchaseAmount ?? 0)} USDT
          </div>
        </div>
        ...
        <div className="rounded-2xl border border-[#26262B] bg-[#16161A] p-6 shadow-lg">
          <div className="mb-4 flex items-center justify-between">
            <span className="text-xs font-semibold uppercase tracking-wider text-[#8E8E93]">기간 총 합계</span>
            <BarChart3 size={18} className="text-[#00D2FF]" />
          </div>
          <div className="font-mono text-2xl font-extrabold text-white">
            {formatUsdt(summary?.totalCombinedAmount ?? 0)} USDT
          </div>
        </div>

        <div className="rounded-2xl border border-[#26262B] bg-[#16161A] p-6 shadow-lg">
          <div className="mb-4 flex items-center justify-between">
            <span className="text-xs font-semibold uppercase tracking-wider text-[#8E8E93]">일평균 합계</span>
            <CalendarRange size={18} className="text-[#FF9F0A]" />
          </div>
          <div className="font-mono text-2xl font-extrabold text-white">
            {formatUsdt(summary?.averageDailyCombinedAmount ?? 0)} USDT
          </div>
        </div>
      </div>
      ...
      {summary?.peakDay && (
        <div className="rounded-xl border border-[#26A17B]/20 bg-[#26A17B]/5 px-4 py-3 text-xs text-[#B7C6CC]">
          최고 합계일: {formatDate(summary.peakDay.date)} / 상품 {formatUsdt(summary.peakDay.totalProductPurchaseAmount)} USDT / 배팅 {formatUsdt(summary.peakDay.totalBetAmount)} USDT
        </div>
      )}
      ...
      <div className="rounded-2xl border border-[#26262B] bg-[#16161A] p-6 shadow-lg">
        <div className="mb-6">
          ...
          <p className="mt-1 text-[11px] text-[#8E8E93]">
            상품구매는 구매일 기준, 배팅은 `REFUNDED` 제외 후 `round_date` 기준으로 합산합니다.
          </p>
        </div>
          ...
          <table className="w-full border-collapse text-left text-xs">
            <thead>
              <tr className="border-b border-[#26262B] font-semibold uppercase tracking-wider text-[#8E8E93]">
                ...
                <th className="px-4 py-3 text-right">상품구매총액</th>
            ...
            <tbody>
              {loading ? (
                <tr>
                  <td colSpan={3} className="py-8 text-center text-[#8E8E93]">
                    일별 배팅 집계를 불러오는 중...
                  </td>
                </tr>
              ) : rows.map((row) => (
                <tr key={row.date} className="border-b border-[#26262B]/40 transition-all hover:bg-[#1C1C21]/30">
                  ...
                  <td className="px-4 py-4 text-right font-mono font-bold text-[#F0B90B]">
                    {formatUsdt(row.totalProductPurchaseAmount)} USDT
                  </td>
